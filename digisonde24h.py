...
#
# example usage:
# python digisonde24h.py directory/TR169_YYYYDDD <output>
#
...
import calendar
import numpy as np
import matplotlib  # not used unless the Agg backend will be used
...
def read_group(group_number, length_of_element):
    group = np.zeros(int(num_elements[group_number - 1]))
...
for filename in glob.glob(sys.argv[1] + '*.SAO'):
    ...
    height.append(list(np.array(o_f2_virt_heights)))
    height.append(list(np.array(o_f1_virt_heights)))
    height.append(list(np.array(o_e_virt_heights)))
    height.append(list(np.array(x_f2_virt_heights)))
    height.append(list(np.array(x_f1_virt_heights)))
    height.append(list(np.array(x_e_virt_heights)))
    height.append(list(np.array(o_es_virt_heights)))
    height.append(list(np.array(o_ea_virt_heights)))

    ff.append(o_f2_frequencies)
    ff.append(o_f1_frequencies)
    ff.append(o_e_frequencies)
    ff.append(x_f2_frequencies)
    ff.append(x_f1_frequencies)
    ff.append(x_e_frequencies)
    ff.append(o_es_frequencies)
    ff.append(o_ea_frequencies)

    length = 0
    length = length + len(list(np.array(o_f2_virt_heights)))
    length = length + len(list(np.array(o_f1_virt_heights)))
    length = length + len(list(np.array(o_e_virt_heights)))
    length = length + len(list(np.array(x_f2_virt_heights)))
    length = length + len(list(np.array(x_f1_virt_heights)))
    length = length + len(list(np.array(x_e_virt_heights)))
    length = length + len(list(np.array(o_es_virt_heights)))
    length = length + len(list(np.array(o_ea_virt_heights)))

    timeaxis.append([hour + (minute + 7.5) / 60.] * length)
...
# write to file if argument is given
if len(sys.argv) == 3:
    savefilename = sys.argv[2]
    fig.savefig(savefilename)

# comment out if you do not want output to screen
plt.show()
